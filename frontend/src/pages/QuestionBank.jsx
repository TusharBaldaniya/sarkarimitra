...
const QuestionBank = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Question List View (Desktop Table / Mobile Responsive Cards) */}
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-semibold border-b border-slate-200 text-[11px]">
                  <tr>
                    <th className="px-6 py-3.5">Question Statement</th>
                  ...
                  {questions.map((q) => (
                    <tr key={q.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-6 py-4 max-w-md">
                        <p className="font-medium text-slate-800 line-clamp-2">{q.questionText}</p>
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {questions.map((q) => (
                <div key={q.id} className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-xl text-slate-600 border border-slate-100">
                    <div>
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => handleEdit(q)}
                        className="p-2 text-slate-500 hover:text-brand-600 bg-slate-100 rounded-lg"
                      >
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => setDeletingId(q.id)}
                        className="p-2 text-slate-500 hover:text-rose-600 bg-slate-100 rounded-lg"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
